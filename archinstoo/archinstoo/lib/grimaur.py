...
def aur_rpc_suggest(prefix: str) -> list[str]:
	...
	if not isinstance(results, list):
		return []
	return [entry for entry in results if isinstance(entry, str)]
...
def search_packages_git(
	*,
	...
	if limit is None:
		limit = 50
	output = run_command(['git', 'ls-remote', '--heads', get_aur_remote()], capture=True)
...
def fetch_package(
	...
	*,
	...
	package_dir = ensure_clone(package, dest_root, refresh=refresh, force_reclone=force, repo_url=repo_url)
...
def build_parser() -> argparse.ArgumentParser:
	...
	inspect_parser.add_argument('--repo-url', help='Inspect package from custom Git URL')

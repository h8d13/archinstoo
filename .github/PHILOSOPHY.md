...
The idea was that with less complixity, we can build more stable expperience and more possible paths.

This is a powerful idea because it would allow for standards to compete in better environnement.
...
This is also to illustrate that for example a feature that only 1% of users actually use should not be given priorities over modular architercture.
...
The first time around where I had done this to the codebase, I had removed too much... This time around, I was more careful, but dtill remoced considerable parts, often in favor of something else. 
...
Anything that is hardcoded is often something that is assumed and kind og get's lost as a standard with no competition.
...
This allows for faster dev builds, more ram space in the ISO, and so on. This also creates a nice seperation of concerns: dev build vs general usage.
...
That it shouldn't be slowed down by some depracated config or some nitpick on wording/styles. 

Dev
---

Whilst dev builds/branched... Well sometimes do some more "crazy" stuff like implementing bcachefs support just, as its dropped out of kernel tree in .18 or supportibg CachyOS kernels, or some random idea that sparked in the shower.

...
## 0.1.15-0
	...
	- `schema.jsonc` becomes `schema.toml`, generated by
	  ...
	  instead of hand-transcribed. `python -m archinstoo --script schema`
	  rewrites it; sections follow the order the install runs them
	  (`scripts/guided.py`, then `minimal_installation`), so reading it
	  top to bottom is reading the install
		- `lib/schema.py` shrinks to "where it lives and how to read
		  it": `tomllib`, stdlib-only, still loadable by path from
		  `nvchecker/NVGEN` with no archinstoo runtime
		- `test_schema_drift.py` (447 lines of transcription pinned
		  against code) is replaced by `test_schema.py`, which fails
		  when the committed file goes stale
	- Terminal is a choice, not a per-profile hardcode. The application
	  menu offers alacritty, foot, kitty, ghostty, konsole, wezterm and
	  xterm, installs exactly one and exports `TERMINAL` in
	  `/etc/environment` (pam_env puts it in the session)
		- i3 and labwc call `*-sensible-terminal`, which try `$TERMINAL`
		  first: nothing to patch. qtile's `guess_terminal()` ignores
		  the variable but walks a fixed list, and every enum value is
		  on it, so installing one resolves to the pick
		- the rest hardcode a binary in a config we own, rewritten at
		  provision time by `swap_terminal`, which warns rather than
		  silently no-op when upstream renames its default
		- xmonad is the holdout: its terminal is compiled into
		  `XMonad/Config.hs` and changing it needs a ghc rebuild
	- `noctalia` profile (noctalia v5 native shell from extra) with a
	  compositor multi-select over niri, hyprland, sway and labwc, niri
	  the default. Per-compositor autostart, IPC binds and layer rules
	  come from the noctalia docs; labwc adds `rc.xml` and autostart.
	  `pyproject` package-data now ships xml and extensionless configs
	  (the sway config was silently unpackaged)
	- dms drops its 16 vendored config files for upstream's
	  `dms setup headless`, verified byte-for-byte against the assets it
	  replaces. It runs per user through `arch_chroot(run_as=)`, writes
	  the `hyprland-session.target` we used to hand-roll, deploys niri's
	  `windowrules.kdl` (which we never copied) and `--skip-existing`
	  warns instead of aborting when a config is already there. Only the
	  terminal differs, so `swap_terminal` repoints the keybind after
		- `dms-greeter` is gone: dms-shell 1.6.0 embedded the UI in the
		  binary and dropped `/usr/share/quickshell/dms`, so the
		  `config.toml` we wrote named a path that no longer exists and
		  greetd hit its start limit every boot. `GreeterType.Tuigreet`
		  replaces it (`greetd-tuigreet` is in extra, draws on the tty,
		  needs no seat group) and both greetd front-ends share one
		  config write keyed on a command
		- assets resynced from upstream's embedded configs first: the
		  `dankdash` ipc verb is `dash toggle` now, so Mod+Y was dead
	- Seat access names the mechanism, not the package
	  (archlinux/archinstall#3467): the choice is `systemd-logind` or
	  `seatd`, shown through `SeatAccess.label` while the value stays the
	  package (it is the nvchecker and saved-config token). Only seatd
	  ships a unit, so `seat_services()` replaces seven identical bodies
	  that enabled a D-Bus-activated polkit with no `[Install]` and
	  printed systemd's "not meant to be enabled". The seat-group
	  `usermod` is centralised in `add_to_seat_group`
	- Boot layouts, each found in a VM and each now unit-tested
		- limine reads FAT12/16/32 and ISO9660 only, but guided
		  UEFI+btrfs mounted the ESP at `/efi` so the kernels stayed in
		  `@`: installed fine, then PANIC'd with "Failed to open kernel"
		  on the graphical console, so serial showed nothing. The FAT
		  rule `layouts.py` already applied for BIOS now covers UEFI
		- UKI is self-contained on the ESP, so naming a separate `/boot`
		  to `bootctl --boot-path` made it demand XBOOTLDR typing for no
		  gain and failed the install after a full pacstrap
		- `_validate_bootloader` only ran from the menu, so `--silent`
		  skipped every check and died at `bootctl` after pacstrap. It
		  is a pure function in `bootloader/validation.py` now, called
		  from both: a 1-2s exit with the disk untouched. Two of its
		  rules were themselves wrong (the XBOOTLDR one ignored UKI; the
		  limine one asked a self-contradictory question and never
		  fired)
		- `boot != efi` compared `_obj_id`, a per-instance uuid4 that
		  takes part in the dataclass `__eq__`, so an identity check was
		  dressed as a value check in two places. One
		  `_has_separate_boot()` on `dev_path` instead
		- `_obj_id` is `str` everywhere: `__post_init__` minted a uuid4
		  while `parse_arg` restored the same identity as a str, so one
		  partition hashed into two buckets depending on where it came
		  from, and `PartitionModification` is a dict key in the luks
		  handling
		- grub-btrfs snapshots: `--boot-directory=<esp>` when `/boot`
		  isn't the ESP, so `grub.cfg` stops landing outside `@` and
		  going stale against the kernel a restored snapshot brings
		  back (#4701, continues #84/#86)
		- refind's kernel directory is extracted from three nested
		  branches into `_refind_kernel_dir`, strings preserved
		  including the leading-backslash asymmetry a real install
		  produces
	- Disk
		- `partition()` removed every mapper `dmsetup ls` returned:
		  installing to a second disk from a host booted off LUKS/LVM
		  would `dmsetup remove --force` the host's own root. `/sys/
		  block/dm-N/slaves/` is the source now, walked to keep only
		  mappers ending on the target disk, deepest first
		- `partition_table` is explicit per device modification
		  (`gpt`/`msdos`, host default when absent), so a config stops
		  changing meaning across machines. `validate_bootloader`
		  rejects bios_grub-on-msdos, BIOS+GPT+grub without bios_grub
		  and MBR with more than 3 partitions before the wipe, where
		  each was a parted traceback mid-partition
		- `disk_encryption` parses from config, so unattended installs
		  can encrypt: `obj_id` references resolve to partitions and LVM
		  volumes, unknown ids fail loud, and a silent install with no
		  password errors instead of hanging. `json()` never writes the
		  plaintext back
		- LUKS opens with `--allow-discards --persistent`, so the flag
		  lands in the LUKS2 header and every consumer inherits it.
		  `fstrim.timer` is enabled unconditionally by
		  `minimal_installation` and was trimming nothing on every
		  encrypted install; btrfs' `discard=async` was dropped the same
		  way. VM: `fstrim -v` moves 61.7 GiB where it moved zero
		  (#897, #1837)
		- lsblk's fstype casing is preserved: af1d4310 lowercased the
		  whole string and the enum spells `crypto_LUKS`
	- Swap and hibernation: the BIOS resume offset is computed in
	  PAGE_SIZE units (`filefrag -b4096`), which the filesystem blocksize
	  only coincidentally matched; a swapfile failure warns instead of
	  aborting; the hibernate swapfile defaults on and zram sizes to RAM
	  with a priority. Models move out of `zram.py` into `swap.py`.
	  Bootloaders take a serial console kernel parameter
	- Nothing waits forever any more. NTP sync and the keyring WKD sync
	  were `while True` loops: a live ISO with a route but no NTP
	  reachability (filtered UDP 123, captive portal) hung before
	  touching the disk. Both take a 60s deadline, then warn and point at
	  `--skip-ntp`/`--skip-wkd`. The reflector wait is deleted outright:
	  archiso v87 stopped enabling `reflector.service`, so the wait
	  exited on its first poll and printed a misleading "completed". Both
	  ISO units are skipped entirely under `h2t`
	- `SysCallError` subclasses `CalledProcessError` and `run()` re-raises
	  into it with returncode, cmd, output and stderr mapped. Found via
	  the sshd profile: `ufw allow ssh` failing in chroot escaped
	  `allow_ssh`'s handler and aborted the whole guided install, and
	  every `except SysCallError` around a list-form `arch_chroot` was
	  dead the same way. Fixed one layer down, not per call site
	- `_run_script` caught `ModuleNotFoundError` and only re-raised when
	  the message mentioned the script module, so a missing import inside
	  a script returned normally: `--script guided` exited 0 having done
	  nothing. It matches on `e.name` now
	- The temporary doas rule for AUR builds is scoped to pacman. The
	  sudoers branch already granted NOPASSWD for `/usr/bin/pacman` only,
	  while doas got `permit nopass <user> as root` with no `cmd`, so the
	  build user held unrestricted root for the whole grimoire run
	- `--silent` works for `packages` and `live`, and the silent/menu flow
	  in `guided`/`live`/`packages` collapses into one `resolve_config`.
	  Running-system flows provision the invoking user
	  (`Os.invoking_username` covers sudo, run0 and doas): `packages`
	  skipped provisioning entirely, `live` falls back to it when auth
	  was skipped
	- Every environment variable is `A2_*` prefixed: `A2_PROFILE_DIR`,
	  `A2_LIVE`, `A2_MINIMAL`, `A2_SILENT_MODE`, `A2_DEPS_FETCHED` and
	  the rest, across `isos/ISOMOD`, the cache script and the bootstrap
	- Hardware: Cirrus amps that split over the ACPI bus are detected by
	  walking module dependencies, which also restores the missing
	  `libvpl`
	- grimoire: `run0` is skipped when `pkcheck` is absent (it auths via
	  polkit, an optdep of systemd) rather than chosen and failed; doas
	  matches `cmd` rules against argv[0] as typed, so the command is
	  resolved through `which` the way makepkg already hands `PACMAN_AUTH`
	  a full path
	- An installed release reports `REL`, not `DEV`: git always fails
	  around site-packages, so the plain fallback labelled every
	  installed package a dev build. `resolve_gitstat(has_pkgbuild)`
	  distinguishes a stamped wheel from a checkout
	- `curses` `color_pair` is gated behind `has_colors`
	- Lint and typing: ruff `ANN` (34 `__init__` return types), `B006`,
	  `B905` strict zips, `RUF015`, `RET504`, `B909`, `C419`, plus pylint
	  `cell-var-from-loop`, `attribute-defined-outside-init`,
	  `raise-missing-from` (28 sites chained), `unspecified-encoding` and
	  `unused-argument`. `pre-commit run --all-files` is green with no
	  path shims, mypy and pylint scoped to `^installer/` minus tests to
	  match CI. Local parted stubs replace the dependency's own
	  (upstream #4735)
	- Examples carry `terminal_config`, the full sample fills
	  `power_management`, `cpu_scheduler`, `development` and `sysctl`,
	  and `test_config_drift` round-trips every example through

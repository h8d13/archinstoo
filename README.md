...
A fork of `archinstall` with only `python-pyparted` as a dependency, many MORE choices, LESS packages installed in end-product, LESS complex flags, and MORE hot-fixes. *Aims to make the code base more readable, maintainable and modifiable by anyone*.
...
Alternatively: We have 2 wrappers/helpers from the repo root: `./RUN`(regular) and `./DEV`(advanced) whcih do similar to step 1 and 2.
